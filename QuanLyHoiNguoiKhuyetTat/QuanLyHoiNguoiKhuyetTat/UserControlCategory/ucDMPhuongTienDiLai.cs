...
{
    public partial class ucDMPhuongTienDiLai : ucBase
    {
        public ucDMPhuongTienDiLai()
        {
            InitializeComponent();
        }

        private void ucDMPhuongTienDiLai_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _selectData()
        {
            ...
            context.DM_PHUONGTIEN_DILAI.Load();
            gcGrid.DataSource = context.DM_PHUONGTIEN_DILAI.Local.ToBindingList();
            _wait.Close();
        }
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colPT_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (gvGrid.FocusedColumn.FieldName == colPT_TEN.FieldName)
            {
                if (string.IsNullOrEmpty(e.Value.ToString().Trim()))
                {
                    e.ErrorText = colPT_TEN.Caption + " không được phép rỗng.";
                    e.Valid = false;
                }
                else if (gvGrid._ValidationSame(colPT_TEN,e.Value +string.Empty))
                {
                    e.ErrorText = colPT_TEN.Caption + " không được trùng.";
                    e.Valid = false;
                }
            }
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colPT_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colPT_TEN.FieldName], colPT_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
