...
{
    public partial class ucDMDungCuHoTro : ucBase
    {
        public ucDMDungCuHoTro()
        {
            InitializeComponent();
        }

        private void ucDMDungCuHoTro_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _selectData()
        {
            ...
            context.DM_DUNGCU_HOTRO.Load();
            gcGrid.DataSource = context.DM_DUNGCU_HOTRO.Local.ToBindingList();
            _wait.Close();
        }
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colDCHT_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (gvGrid.FocusedColumn.FieldName == colDCHT_TEN.FieldName)
            {
                if (string.IsNullOrEmpty(e.Value.ToString().Trim()))
                {
                    e.ErrorText = colDCHT_TEN.Caption + " không được phép rỗng.";
                    e.Valid = false;
                }
                else if (gvGrid._ValidationSame(colDCHT_TEN,e.Value +string.Empty))
                {
                    e.ErrorText = colDCHT_TEN.Caption + " không được trùng.";
                    e.Valid = false;
                }
            }
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colDCHT_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colDCHT_TEN.FieldName], colDCHT_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
