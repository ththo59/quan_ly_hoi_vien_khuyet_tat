...
{
    public partial class ucDMNKTSongVoiAi : ucBase
    {
        public ucDMNKTSongVoiAi()
        {
            InitializeComponent();
        }

        private void ucDMNKTSongVoiAi_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _selectData()
        {
            ...
            context.DM_NOI_O_SONG_VOI.Load();
            gcGrid.DataSource = context.DM_NOI_O_SONG_VOI.Local.ToBindingList();
            _wait.Close();
        }
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colNOSV_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (gvGrid.FocusedColumn.FieldName == colNOSV_TEN.FieldName)
            {
                if (string.IsNullOrEmpty(e.Value.ToString().Trim()))
                {
                    e.ErrorText = colNOSV_TEN.Caption + " không được phép rỗng.";
                    e.Valid = false;
                }
                else if (gvGrid._ValidationSame(colNOSV_TEN,e.Value +string.Empty))
                {
                    e.ErrorText = colNOSV_TEN.Caption + " không được trùng.";
                    e.Valid = false;
                }
            }
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colNOSV_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colNOSV_TEN.FieldName], colNOSV_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
