...
{
    public partial class ucDMTrinhDoHocVan : ucBase
    {
        public ucDMTrinhDoHocVan()
        {
            InitializeComponent();
        }

        private void ucDMTrinhDoHocVan_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colHV_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colHV_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colHV_TEN.FieldName], colHV_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
