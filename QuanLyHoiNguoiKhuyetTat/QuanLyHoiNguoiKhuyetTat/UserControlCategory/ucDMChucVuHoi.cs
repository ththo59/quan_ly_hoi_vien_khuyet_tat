...
{
    public partial class ucDMChucVuHoi : ucBase
    {
        public ucDMChucVuHoi()
        {
            InitializeComponent();
        }

        private void ucDMChucVuHoi_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _selectData()
        {
            ...
            context.DM_CHUCVU_HOI.Load();
            gcGrid.DataSource = context.DM_CHUCVU_HOI.Local.ToBindingList();
            _wait.Close();
        }
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colCV_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (gvGrid.FocusedColumn.FieldName == colCV_TEN.FieldName)
            {
                if (string.IsNullOrEmpty(e.Value.ToString().Trim()))
                {
                    e.ErrorText = colCV_TEN.Caption + " không được phép rỗng.";
                    e.Valid = false;
                }
                else if (gvGrid._ValidationSame(colCV_TEN,e.Value +string.Empty))
                {
                    e.ErrorText = colCV_TEN.Caption + " không được trùng.";
                    e.Valid = false;
                }
            }
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colCV_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colCV_TEN.FieldName], colCV_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
