...
{
    public partial class ucDMTrinhDoChuyenMon : ucBase
    {
        public ucDMTrinhDoChuyenMon()
        {
            InitializeComponent();
        }

        private void ucDMTrinhDoChuyenMon_Load(object sender, EventArgs e)
        {
            initData();
        ...
        void _selectData()
        {
            ...
            context.DM_TRINH_DO_CHUYEN_MON.Load();
            gcGrid.DataSource = context.DM_TRINH_DO_CHUYEN_MON.Local.ToBindingList();
            _wait.Close();
        }
        ...
        void _deleteData()
        {
            ...
            string Ten = gvGrid.GetRowCellValue(gvGrid.FocusedRowHandle, colCM_TEN.FieldName).ToString();
        ...
        private void gvGrid_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            if (gvGrid.FocusedColumn.FieldName == colCM_TEN.FieldName)
            {
                if (string.IsNullOrEmpty(e.Value.ToString().Trim()))
                {
                    e.ErrorText = colCM_TEN.Caption + " không được phép rỗng.";
                    e.Valid = false;
                }
                else if (gvGrid._ValidationSame(colCM_TEN,e.Value +string.Empty))
                {
                    e.ErrorText = colCM_TEN.Caption + " không được trùng.";
                    e.Valid = false;
                }
            }
        ...
        private void gvGrid_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            e.Valid = true;
            if ((gvGrid.GetRowCellValue(e.RowHandle, colCM_TEN.FieldName) + string.Empty).Trim().Length == 0)
            {
                gvGrid.SetColumnError(gvGrid.Columns[colCM_TEN.FieldName], colCM_TEN.Caption + " không được phép rỗng.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Default);
                e.Valid = false;
            }
        }
        #endregion
