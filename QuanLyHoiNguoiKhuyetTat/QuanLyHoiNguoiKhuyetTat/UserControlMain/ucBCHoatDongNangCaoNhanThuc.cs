...
{
    public partial class ucBCHoatDongNangCaoNhanThuc : ucBase
    {
        XtraReport rptGlobal = new XtraReport();
        public ucBCHoatDongNangCaoNhanThuc()
        {
            InitializeComponent();
        }

        private void ucBCHoatDongNangCaoNhanThuc_Load(object sender, EventArgs e)
        {
        ...
        private void _loadData()
        {
            ...
            var listDMHoiThao = FuncCategory.loadDMHoiThao();
            ...
            foreach (QL_HOATDONG_HOITHAO row in data)
            {
                ...
                item.HD_ID = row.HT_ID;
                item.HD_TEN = row.HT_TEN;
                item.HD_THOIGIAN_BATDAU = row.HT_THOIGIAN_BATDAU;
                item.HD_THOIGIAN_KETTHUC = row.HT_THOIGIAN_KETTHUC;
                item.HD_THOIGIAN = FunctionHelper.formatFromDateToDate(row.HT_THOIGIAN_BATDAU, row.HT_THOIGIAN_KETTHUC);
                var dm = listDMHoiThao.Where(p => p.ID == row.HT_LOAI_ID).First();
                if(dm != null)
                {
                    item.HD_LOAI = dm.NAME;
                }
                item.HD_NOIDUNG = row.HT_NOIDUNG;
                lists.Add(item);
            }

            rptBCTongKetHoatDong_NangCaoNhanThuc rpt = new rptBCTongKetHoatDong_NangCaoNhanThuc();
